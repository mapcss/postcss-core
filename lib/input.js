...
import { fromFileUrl, toFileUrl } from "./deps.js";
...
class Input {
  ...
  error(message, line, column, opts = {}) {
    ...
    if (this.file) {
      result.input.url = toFileUrl(this.file).toString();
  ...
  origin(line, column, endLine, endColumn) {
    ...
    if (isAbsolute(from.source)) {
      fromUrl = toFileUrl(from.source);
    } else {
      fromUrl = new URL(
        from.source,
        this.map.consumer().sourceRoot || toFileUrl(this.map.mapFile)
      );
    }
    ...
    if (fromUrl.protocol === "file:") {
      result.file = fromFileUrl(fromUrl);
    }
